...
// Слигком часто сообщения высокой температуры - Итоговый умный код с комбинированием всех функций Интеллектуальная отправка сообщений и Умный таймер с адаптивной задержкой и Комплексное умное собщение
...
#define VOLTAGE_HYSTERESIS 2.0
#define MAX_HYSTERESIS 5.0
#define MIN_HYSTERESIS 1.0
const unsigned long VOLTAGE_ANOMALY_DURATION = 10000; // 10 секунд базовая задержка
...
void checkSensors() {
  ...
  if (currentMillis - previousMillisCheckSensors >= interval) {
      ...
      tempControllerIndex = 0; humControllerIndex = 0;
      tempGeneratorIndex = 0; humGeneratorIndex = 0;
      tempControllerCount = 0; humControllerCount = 0;
      tempGeneratorCount = 0; humGeneratorCount = 0;
  ...
  if (currentMillis - previousMillisCheck >= checkInterval && checkCount < MAX_READINGS) {
      ...
      if (checkCount == MAX_READINGS - 1) { // checkCount == 9 при MAX_READINGS = 10
...
void start_generator() {
  ...
  if (zmpt_generator.getRmsVoltage() < 50 && start_reley_count > 0) {
    ...
    if (start_from_tg == 1) start_from_tg = 0;
    ...
    if (bot.messages[i].type == "callback_query") {
      ...
      bot.sendMessage(bot.messages[i].from_id, bot.messages[i].text, "");
        ...
        if (text == "/gen_start" || text == "/gen_start@" + String(NAME_BOT)) {
          if (start_from_tg == 0) { // Проверка на случай двоного запуска
...
bool shouldSendAlert(const VoltageAlert& alert, unsigned long currentMillis, unsigned long adaptiveDuration) {
  // Критическое превышение - отправляем сразу
  if (alert.maxVoltage > VOLTAGE_HIGH_THRESHOLD + 10.0) {
      return true;
  }
  ...
  // Сильные колебания - отправляем раньше
  if (alert.maxVoltage - alert.minVoltage > 5.0 && alert.fluctuationCount > 5) {
      return true;
  }
...
void sendSmartAlert(const VoltageAlert& alert, const String& source) {
  String message = "⚡️ ";
  
  if (alert.maxVoltage > VOLTAGE_HIGH_THRESHOLD) {
      message += "Высокое напряжение " + source + ": ";
  } else {
      message += "Низкое напряжение " + source + ": ";
  }
  
  message += "Мин: " + String(alert.minVoltage, 1) + "V, ";
  message += "Макс: " + String(alert.maxVoltage, 1) + "V, ";
  message += "Колебаний: " + String(alert.fluctuationCount);
  
  // Добавляем продолжительность
  unsigned long duration = (millis() - alert.startTime) / 1000;
...
void checkVoltage(float networkVoltage, float generatorVoltage) {
  ...
  if (networkVoltage > VOLTAGE_HIGH_THRESHOLD) {
      ...
      if (shouldSendAlert(voltageAlertNetwork, currentMillis, adaptiveAnomalyDuration) && 
          ...
          sendSmartAlert(voltageAlertNetwork, "сети");
  ...
  // 5. Аналогично для других проверок (низкое напряжение, генератор)
  // Проверка низкого напряжения СЕТИ
  if (networkVoltage > 50 && networkVoltage < VOLTAGE_LOW_THRESHOLD) {
      // ... аналогичная логика ...
  }
  
  // Проверка напряжения ГЕНЕРАТОРА
  if (generatorVoltage > 150) {
      if (generatorVoltage > VOLTAGE_HIGH_THRESHOLD) {
          // ... аналогичная логика для генератора ...
      }
      if (generatorVoltage < VOLTAGE_LOW_THRESHOLD) {
          // ... аналогичная логика для генератора ...
      }
  } else {
...
void setup() {
  ...
  /*server.on("/sensor-data", HTTP_GET, [](AsyncWebServerRequest *request) {
    if (!request->hasArg("sensor") || !request->hasArg("range")) {
      request->send(400, "text/plain; charset=utf-8", "Требуются параметры: sensor и range");
      return;
    }
    
    String sensor = request->arg("sensor"); // "temp1", "hum2" и т.д.
    String range = request->arg("range");   // "24h" или "1h"
    
    DynamicJsonDocument doc(8192);
    JsonArray labels = doc.createNestedArray("labels");
    JsonArray values = doc.createNestedArray("values");
  
    // Сначала собираем все подходящие записи в вектор
    std::vector<SensorRecord> filteredRecords;
    
    File file = SPIFFS.open(SENSOR_DATA_FILE, "r");
    while (file.available()) {
      SensorRecord record;
      file.read((uint8_t*)&record, sizeof(record));
      
      if (isInRange(record.timestamp, range)) {
    ...
    for (auto it = filteredRecords.rbegin(); it != filteredRecords.rend(); ++it) {
      const SensorRecord& record = *it;
      labels.add(formatTime(record.timestamp));
      
      if (sensor == "temp1") values.add(record.temp_controller / 10.0f);
      else if (sensor == "hum1") values.add(record.hum_controller);
      else if (sensor == "press1") values.add(record.press_controller / 100.0f);
      else if (sensor == "temp2") values.add(record.temp_generator / 10.0f);
      else if (sensor == "hum2") values.add(record.hum_generator);
      else if (sensor == "press2") values.add(record.press_generator / 100.0f);
      else if (sensor == "volt_net") values.add(record.voltage_network / 10.0f);
      else if (sensor == "volt_gen") values.add(record.voltage_generator / 10.0f);
      else if (sensor == "gpio") values.add(record.gpio_states);
    }
...
void loop() {
  ...
  if (currentMillis - lastConnectCheck > 300000  || currentMillis - start_time < 120000) {  // Проверять подключен ли WiFi 2 минуты при старте и каждые 5 минут далее
...
// Запуск генеатора
  if (networkVoltage < 50) {
          ...
          if (currentTime_noVoltage - noVoltageStartTime >= NO_VOLTAGE_DURATION) {
            if (start_reley_count == 7) {
              ...
              start_generator();
                ...
                if (currentMillis - lastSentAttemptTime >= 10000) { // Если прошло 10 секунд
    ...
    } else {   // Генератор запущен, включаем зелёный
        if (digitalRead(PIN_Gas_Valve) == HIGH) {
  ...
  if (networkVoltage > 50) {
    ...
    } else { // Остановка генератора если есть питание из сети и реле клапана бензина включено, иначе мигает белый
        if (!alertSent_NO_GOROD) { // Если еще не уведомляли о появлении напряжения
              bot.sendMessage(CHAT_ID, "\U00002705 Появилось напряжение с города", "");
              logEvent("Появилось напряжение с города");
